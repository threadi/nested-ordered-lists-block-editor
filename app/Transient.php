...
class Transient {
	...
	public function display(): void {
		...
		if ( $this->has_message() ) {
			?>
			<div class="nolg-transient updated <?php echo esc_attr( $this->get_type() ); ?>" data-dismissible="<?php echo esc_attr( $this->get_name() ); ?>-<?php echo absint( $this->get_dismissible_days() ); ?>">
				<h3><?php echo esc_html( Helper::get_plugin_name() ); ?></h3>
				<?php
	...
	private function get_admin_transient_dismiss_cache(): string|int|false {
		$cache_key = 'nolg-dismissed-' . md5( $this->get_name() );
	...
	public function delete_dismiss(): void {
		delete_option( 'nolg-dismissed-' . md5( $this->get_name() ) );
	}
	...
	public function get_hide_on(): array {
		 ...
		 */
		return apply_filters( 'nolg_transient_hide_on', $hide_on, $this );
	}
